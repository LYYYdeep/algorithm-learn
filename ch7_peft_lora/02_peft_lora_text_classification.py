...
lora_config = LoraConfig(
    r=8,                      # LoRA 秩
    lora_alpha=32,             # 缩放因子 alpha，通常是 2*r
    target_modules=["query", "value"],  # 只给 Q 和 V 加 LoRA，原始论文就是这么做的
    lora_dropout=0.05,
    bias="none",                # 不训练 bias
    task_type="SEQ_CLS",        # 任务类型：序列分类
)

# 给模型加上 LoRA，得到 PEFT 模型
...
print(f"Rank r={lora_config.r}, Target modules={lora_config.target_modules}")
print("="*60 + "\n")
...
for epoch in range(num_epochs):
    ...
    if val_acc > best_val_acc:
        ...
        model.save_pretrained("./lora-bert-chinese-sentiment-best")
...
print("LoRA model saved to ./lora-bert-chinese-sentiment-best")